...
<div class="container">
    ...
    <form action="{{ route('apply-loker.store-tahapan', ['id' => $applyLoker->id]) }}" method="POST">
        @csrf
        <div class="mb-6">
            <label for="name" class="form-label">Pencari Kerja</label>
            <input type="text" id="name" value="{{ $applyLoker->pencaker->nama }}" class="form-input" required disabled>
        </div>
        <div class="mb-6">
            <label for="gender" class="form-label">Jenis Kelamin</label>
            <input type="input" id="gender" value="{{ $applyLoker->pencaker->jenis_kelamin }}" class="form-input" required disabled>
        </div>
        <div class="mb-6">
            <label for="telpon" class="form-label">No. Telepon</label>
            <input type="input" id="telpon" value="{{ $applyLoker->pencaker->no_telp }}" class="form-input" required disabled>
        </div>
        <div class="mb-6">
            <label for="job" class="form-label">Pekerjaan yang Dilamar</label>
        ...
        <div class="mb-6">
            <label for="cptelpon" class="form-label">No. Telepon CP</label>
            <input type="input" id="cptelpon" value="{{ $applyLoker->loker->no_telp_cp }}" class="form-input" required disabled>
        </div>
        <button type="submit" name="status" value="1" class="blue-button">Lulus Administrasi</button>
        <button type="submit" name="status" value="0" class="blue-button">Tidak Lulus Administrasi</button>
